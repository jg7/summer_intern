...
class Model(nn.Module):
    ...
    def forward(self, input, target, batch, vocab, train=True):
        ...
        for i in range(tar_seq-1):
            ...
            attn = F.softmax(attn2.view(b,in_seq),dim=-1) # [b x in_seq]
            ...
            p_vocab = F.softmax(self.V2(self.V1(torch.cat([state.squeeze(),context],1))),dim=-1) # [b x vocab]
            ...
            zero_val = 0
            while zero_val in set_numbers:
                set_numbers.remove(zero_val)
            c = Counter(numbers)
